...
// Creazione tabella bufale (inclusa la colonna 'carattere' per i Trend)
db.exec(`
...
// Migration automatica nel caso la colonna 'carattere' non esistesse in un DB già attivo
try {
...
if (countCheck.count === 0) {
  const insertInit = db.prepare('INSERT INTO bufale (domanda, risposta, carattere, voti) VALUES (?, ?, ?, ?)');
  insertInit.run(
...
app.post('/api/classifica/vota', limiterVoti, (req, res) => {
  const { id, domanda, risposta, carattere = 'auto' } = req.body;
  ...
  try {
    ...
    if (domanda && risposta) {
      ...
      if (esistente) {
        ...
        const insertStmt = db.prepare('INSERT INTO bufale (domanda, risposta, carattere, voti) VALUES (?, ?, ?, 1)');
        const info = insertStmt.run(domanda, risposta, carattere);
...
// NEW API: Calcolo Dinamico dei Trend della Disinformazione
